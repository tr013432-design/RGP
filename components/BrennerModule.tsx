...
interface Lead {
  ...
  company: string;
  value: number;
  status: 'PROSPECÇÃO' | 'QUALIFICAÇÃO' | 'PROPOSTA' | 'FECHAMENTO' | 'NÃO INTERESSADOS';
  email: string;
  phone: string;
  location: string;
  source: string;
            ...
            <div className="flex-1 space-y-3">
              {leads.filter(l => l.status === col).map(lead => (
                <div
                  ...
                  className={`bg-slate-800 p-4 rounded-lg border cursor-grab active:cursor-grabbing hover:shadow-lg transition-all group relative ${
                ...
                >
                  ...
                  {lead.company && (
                    <p className="text-xs text-slate-400 truncate mb-2">{lead.company}</p>
                  )}
              ...
              <button
                ...
                className={`w-full py-3 border border-dashed rounded-lg text-xs transition-all mt-2 ${
                  col === 'NÃO INTERESSADOS'
                    ? 'border-red-900/40 text-red-300 hover:border-red-500 hover:text-red-400'
        ...
        <div className="lg:col-span-2 bg-slate-900 p-6 rounded-xl border border-slate-800">
          <div className="flex justify-between mb-4">
            <h3 className="font-bold text-white flex items-center gap-2">
              <i className="fas fa-bolt text-emerald-500"></i> Brenner Scripts
            </h3>
            <button
              onClick={() => setShowSavedScripts(!showSavedScripts)}
              className="lg:hidden text-xs text-emerald-400 border border-emerald-900 bg-emerald-900/20 px-2 py-1 rounded"
            >
              Ver Salvos
            </button>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {['Tá caro', 'Vou pensar', 'Falar com sócio', 'Não tenho tempo'].map(opt => (
                <button
                  key={opt}
                  onClick={() => setObjection(opt)}
                  className="text-xs bg-slate-800 border border-slate-700 p-2 rounded hover:bg-slate-700 text-slate-300 transition-colors"
                >
                  {opt}
                </button>
              ))}
            ...
            <button
              ...
              className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg transition-all text-sm"
            >
            ...
            <div className="mt-6 bg-slate-950 border border-slate-800 p-4 rounded-lg">
              <div className="flex justify-end mb-2">
                <button
                  ...
                  className="text-xs flex items-center gap-2 text-emerald-400 hover:text-white"
                >
        ...
        <div className={`lg:col-span-1 bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col ${showSavedScripts ? 'block' : 'hidden lg:flex'}`}>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar max-h-[400px]">
            {savedScripts.length === 0 ? (
              <p className="text-xs text-slate-600 text-center italic mt-4">
                Nenhum script salvo.
              </p>
            ) : (
              savedScripts.map(s => (
                <div key={s.id} className="bg-slate-950/50 p-3 rounded border border-slate-800 flex justify-between items-start group">
                  <div
                    onClick={() => {
                      ...
                      setObjection(s.title);
                    }}
                    className="cursor-pointer flex-1"
                  >
                    <p className="text-xs font-bold text-slate-300 group-hover:text-emerald-400">
                      {s.title}
                    </p>
              ...
              <div className="flex gap-2">
                {editingLeadId && (
                  <button
                    ...
                    className="px-3 py-1.5 bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white rounded text-xs font-bold border border-red-500/20"
                  >
